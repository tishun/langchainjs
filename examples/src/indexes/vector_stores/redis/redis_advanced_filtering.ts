...
const vectorStore = await RedisVectorStore.fromDocuments(
  ...
  new OpenAIEmbeddings(),
  {
    ...
    metadataSchema: [
      ...
      { name: "created_at", type: "timestamp", options: { sortable: true } },
      { name: "brand", type: "tag" },
...
timestampResults.forEach((doc) =>
  console.log(
    `- ${doc.metadata.title} (${doc.metadata.created_at.toDateString()})`
  )
);
console.log();
...
// Example 10: Backward compatibility with legacy filters
console.log("10. Legacy filtering - Array of strings (backward compatible):");
const legacyResults = await vectorStore.similaritySearch(
  "products",
  5,
  ["electronics", "books"] // Legacy array format
);
console.log(`Found ${legacyResults.length} items using legacy filter`);
legacyResults.forEach((doc) =>
  console.log(`- ${doc.metadata.title} (${doc.metadata.category})`)
);
